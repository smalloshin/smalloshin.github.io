...
resource "google_cloud_run_v2_service" "api" {
  ...
  template {
    ...
    containers {
      ...
      env { name = "GCP_REGION"           value = var.gcp_region }
      env { name = "CORS_ORIGIN"          value = var.cors_origins }
      env { name = "CLOUDFLARE_ZONE_ID"   value = var.cloudflare_zone_id }
      env { name = "CLOUDFLARE_ZONE_NAME" value = var.cloudflare_zone_name }
      env { name = "SHARED_REDIS_HOST"    value = google_compute_instance.shared_redis.network_interface[0].network_ip }
      env { name = "SHARED_REDIS_PORT"    value = "6379" }
      env {
        name  = "DATABASE_URL"
